...
testExamplesWithCode :: [((String,[Int]), ExampleOutput)] -> [(Bool, ExampleOutput, ExampleOutput)]
...
examplesIO = 
  ...
  -- check if input is 8
  , ("3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99", [7],[0999])
...
testExamplesIO examples =
  let 
    (code, input, expectedOutput) = unzip3 examples
    cases = zip code input
    results = map output $ map runComputer $ map (\(code, input) -> newComputer (parseProgram code) input) cases
    checks = map (\(e, r) -> e == r) $ zip expectedOutput results 
  in
    zip3 checks expectedOutput results

test_code :: Bool
test_code = 
  let 
    (checks,_,_)   = Prelude.unzip3 $ testExamplesWithCode examplesWithCode
