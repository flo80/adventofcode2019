...
day6a :: String -> Int
day6a contents =
  let lineItems = lines $ contents
      allPairs  = map splitOrbitString lineItems
  in  recurseOrbits allPairs

splitOrbitString :: String -> (String, String)
splitOrbitString s = let (a : b : _) = splitOn ")" s in (a, b)

recurseOrbits :: [(String, String)] -> Int
recurseOrbits allPairs = recurseOrbits' allPairs 0 "COM"

recurseOrbits' :: [(String, String)] -> Int -> String -> Int
recurseOrbits' allPairs level currentObject =
  let children =
          filter (\(a, _) -> a == currentObject) allPairs :: [(String, String)]
  in
    case (length children) of
      0 -> level
      _ ->
        level
          + (sum $ map (\(_, c) -> recurseOrbits' allPairs (level + 1) c)
                       children
            )
...
day6b contents =
  ...
  in  length youPred + length sanPred - 2 * common - 2
...
getAllPredecessors allPairs currentObject
  ...
  where parent = fst $ head $ filter (\(_, b) -> b == currentObject) allPairs
...
newSolarSystem allPairs = buildOrbital' allPairs 0 "COM"

buildOrbital' :: [(String, String)] -> Int -> String -> OrbitalObject
buildOrbital' allPairs predecessors currentObject =
  let childrenList = filter (\(a, _) -> a == currentObject) allPairs
      children = map (\(_, c) -> buildOrbital' allPairs (predecessors + 1) c)
                     childrenList
  in  OrbitalObject { name         = currentObject
...
listOrbitNumbers' com
  | (length $ children com) == 0
  = [(name com, predecessors com)]
  | otherwise
  = [(name com, predecessors com)]
    ++ (concatMap listOrbitNumbers' $ children com)
